...
Both of the distributions are not normally distributed as shown by the dashed normal distributions (with same mean/variance) shown in  figure 1. This introduces some complexity in producing confidence intervals. For the time caught confidence interval it is possible to closely approximate the normal distribution by transforming the data, e.g. applying an exponent of approximately 0.79 removes the bias.
...
print(xtable(rbind(SizeConfidence, TimeConfidence), caption="Bootstrapped confidence intervals for the mean (95%)"), include.colnames=F, digits = 4)
@

Note: At 3 significant figures the students t test yields the same confidence intervals, for applications where accuracy is only required at this level the students t test or normal approximation is sufficient.
\subsection{Codependency}
...
sizeIntervalPlot <- ggplot(intervals, aes(x = V3, y = intervalSize)) + geom_bar(stat = "identity") +
  ...
  labs(x = "Hour Interval", y = "Mean Fish Size", title = "Mean fish size each Hour")
timeIntervalPlot <- ggplot(intervals, aes(x = V3, y = intervalFreq)) + geom_bar(stat = "identity") +
  theme(axis.text.x=element_text(hjust=2)) +
  labs(x = "Hour Interval", y = "Frequency of fish caught", title = "Number of fish caught each Hour")

jpeg("figure4.jpg", width = 360, height=240)
grid.arrange(sizeIntervalPlot, timeIntervalPlot)
dev.off()
@

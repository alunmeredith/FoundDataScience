...
<<Intervals plot, echo=F, results=hide>>=
...
sizeIntervalPlot <- ggplot(intervals, aes(x = V3, y = intervalSize)) + geom_bar(stat = "identity") +
  ...
  labs(x = "Hour Interval (start time)", y = "Mean Fish Size", title = "Mean fish size Hourly")
timeIntervalPlot <- ggplot(intervals, aes(x = V3, y = intervalFreq)) + geom_bar(stat = "identity") +
  scale_x_continuous(label=function(x){return(paste(x, ":00"))}) +
  labs(x = "Hour Interval (start time)", y = "Frequency of fish caught", title = "Number of fish caught Hourly")
...
Visually Figure 1 (bottom left), showing a scatter plot of time of fish vs size of day demonstrates no obvious correlation between time of catch and size of catch. The covariance between the variables is \Sexpr{round(covariance[2],2)}, this is difficult to interpret so we use a Pearson product-moment correlation yielding, \Sexpr{round(correlationTest$estimate,2)} (\Sexpr{ round(attr(correlationTest$conf.int, "conf.level"),2)} \% CI: \Sexpr{round(correlationTest$conf.int[1],2)} to \Sexpr{round(correlationTest$conf.int[2],2)}).
...
<<linearRegression, echo=F, results=tex>>=
linearReg = lm(size ~ times, data=fish)
coef = coefficients(linearReg)
@

We can also use a linear regression model yields $ y = \Sexpr{round(coef[2],2)}x + \Sexpr{round(coef[1],2)} $, with a p value of 0.704 which is typically too high to reject the null hypothesis. Similarly the 95\% confidence intervals overlap 0. 
